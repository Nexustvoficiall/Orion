...
// NOVAS VARIÁVEIS PARA DADOS DO BANNER FINAL (Adicionado)
let currentGenero = '';
let currentAno = '';
let currentDuracao = '';
...
    // Ano
    metadata.ano = (item.release_date || item.first_air_date || '').substring(0, 4);
    
    // Gênero (Pega o primeiro, se existir)
    if (item.genres && item.genres.length > 0) {
        metadata.genero = item.genres[0].name;
    } else {
        metadata.genero = '';
    }

    // Duração (runtime para filme, episode_run_time para série)
    const runtime = item.runtime || (item.episode_run_time ? item.episode_run_time[0] : null);
    
    if (runtime && runtime > 0) {
        const hours = Math.floor(runtime / 60);
        const minutes = runtime % 60;
        metadata.duracao = `${hours > 0 ? hours + 'h' : ''}${minutes > 0 ? minutes + 'm' : ''}`;
    } else {
        metadata.duracao = '';
    }
...
    // Reset visual e metadados (CORRIGIDO: adicionado reset de metadados)
    posterEl.src = "";
...
    // EXTRAÇÃO E ARMAZENAMENTO DE METADADOS (CORRIGIDO)
    const metadados = formatarMetadados(item);
    currentGenero = metadados.genero;
    currentAno = metadados.ano;
    currentDuracao = metadados.duracao;
...
    // Aqui você chamaria a API /api/gerar-banner real
    // Neste momento, estamos apenas simulando o preview com a imagem do Cloudinary
    
    console.log(`Preview pronto: ${corSelecionada} | Metadados: Gênero=${currentGenero}, Ano=${currentAno}, Duração=${currentDuracao}`);
  } catch (err) {
